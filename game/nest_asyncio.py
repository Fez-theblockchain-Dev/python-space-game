...
def run_main(async_main):
    """
    Run an async main entry point (e.g. main_menu).
    Works in both browser (Pygbag/Emscripten) and desktop.
    In browser: patches the event loop and runs on the existing loop.
    In desktop: uses asyncio.run().
    """
    coro = async_main() if asyncio.iscoroutinefunction(async_main) else async_main
